...
abstract class AbstractSegmentWriter
{
    ...
    public function addField(Field $field): int
    {
        if (!isset($this->_fields[$field->name])) {
            ...
            $this->_fields[$field->name] = new FieldInfo($field->name,
                $field->isIndexed,
                $fieldNumber,
    ...
    public function addFieldInfo(FieldInfo $fieldInfo): int
    {
        if (!isset($this->_fields[$fieldInfo->name])) {
            $fieldNumber = count($this->_fields);
            $this->_fields[$fieldInfo->name] = new FieldInfo($fieldInfo->name,
                $fieldInfo->isIndexed,
                $fieldNumber,
    ...
    public function addTerm($termEntry, $termDocs): void
    {
        ...
        $term = new Term($termEntry->text, $this->_fields[$termEntry->field]->number);
